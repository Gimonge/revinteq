<template>
  <div style="min-height:100vh;background:var(--bg);display:flex;align-items:center;justify-content:center;font-family:var(--font)">
    <div style="background:var(--card);border-radius:var(--r-xl);border:1px solid var(--border);box-shadow:var(--shadow-lg);padding:40px;width:100%;max-width:400px">
      <div style="text-align:center;margin-bottom:28px">
        <div class="rv-logo-mark" style="font-size:26px;justify-content:center;display:flex;gap:8px">
          <span>Revinteq</span>
          <span class="rv-admin-badge">Admin</span>
        </div>
        <div style="font-size:13px;color:var(--slate-light);margin-top:8px;font-weight:600">Gimsc Solutions Ltd — Nakuru</div>
      </div>
      <div v-if="error" class="rv-al rv-al-r" style="margin-bottom:16px">{{ error }}</div>
      <div class="rv-fg"><label class="rv-fl">Email or Username</label><input v-model="email" class="rv-fi" type="text" placeholder="admin@gimsc.co.ke" @keyup.enter="login"></div>
      ...
      <button class="rv-btn rv-btn-p" style="width:100%;justify-content:center;margin-top:4px" @click="login" :disabled="loading">
        ...
        <span v-else><i class="ti ti-lock" aria-hidden="true"></i> Sign In to Admin</span>
      </button>
    </div>
  </div>
</template>
